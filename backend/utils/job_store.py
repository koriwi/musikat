...
def get_album_aggregate(album_id: str, *, exclude_job_id: Optional[str] = None) -> dict:
    ...
    try:
        ...
        cancelled = conn.execute(
            f"SELECT COUNT(*) AS n FROM download_jobs WHERE {where} AND status = 'cancelled'",
            params,
        ).fetchone()["n"]
        ...
        current = conn.execute(
            ...
            SELECT job_id
            FROM download_jobs
            WHERE {where} AND status NOT IN ('completed', 'error', 'cancelled')
        ...
        if total > 0 and terminal >= total:
            status = "cancelled" if cancelled == total else "completed"
        else:
            status = "downloading"
...
def list_jobs(limit: int = 200) -> list[dict]:
    """Return the most recently updated download jobs, newest first."""
    conn = _db()
    try:
        rows = conn.execute(
            "SELECT * FROM download_jobs ORDER BY updated_at_ms DESC LIMIT ?",
            (limit,),
        ).fetchall()
        jobs = []
        for r in rows:
...
def is_job_cancelled(job_id: str) -> bool:
    """True if a job has status 'cancelled'."""
    conn = _db()
    try:
        row = conn.execute(
...
def cancel_job(job_id: str) -> bool:
    ...
    For an album meta job (job_id starts with 'album:'), cancel all of its
    queued or in-progress track jobs too. Completed and failed jobs stay
    unchanged.
    """
    conn = _db()
    try:
        row = conn.execute(
            "SELECT status, album_id FROM download_jobs WHERE job_id = ?", (job_id,)
        ).fetchone()
        if not row:
            return False
        ...
        ids = [job_id]
        if job_id.startswith("album:") and row["album_id"]:
            track_rows = conn.execute(
                "SELECT job_id FROM download_jobs WHERE album_id = ? AND status NOT IN ('completed', 'error', 'cancelled')",
                (row["album_id"],),
            ).fetchall()
            ids += [t["job_id"] for t in track_rows]

        now = _now_ms()
        for jid in set(ids):
            conn.execute(
                "UPDATE download_jobs SET status='cancelled', message='Cancelled', stage='cancelled', progress=0, updated_at_ms=? "
