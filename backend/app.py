...
class DownloadRequest(BaseModel):
    ...
    force: Optional[bool] = False  # Skip duplicate checks (for re-downloads/upgrades)
...
@app.get("/api/jobs")
async def list_download_jobs(limit: int = Query(200, ge=1, le=1000)):
    """List recent download jobs, newest first.

    The Lidarr plugin polls this endpoint to show download progress and
    discover completed files for import.
    """
...
async def download_album(request: AlbumDownloadRequest, background_tasks: BackgroundTasks):
    ...
    for track in album["tracks"]:
        ...
        if request.force or dup is None:
